...
- Live & bootstrapped on port **5550**, connected to MongoDB Atlas (`bennie-connect`). Implemented: `main.ts`, `app.module.ts`, `config`, `database`, **users**, **auth** (register / login / google / refresh / logout / me / forgot-password / reset-password), **mail** (OneSignal). Most other domain modules and the whole admin module are **PRD-only, not built yet**.
...
- For live changes, start the server and smoke-test the affected endpoints against `http://localhost:5550/api/v1`; paste the real output. If `:5550` is already held by another process, say so.
